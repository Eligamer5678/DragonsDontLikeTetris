...
export class multimodifierScene extends Scene {
    ...
    sendState(localDragon){
        if (this.server) {
            ...
            if (now - this.lastStateSend > 20) {
                ...
                diff[this.playerId + 'scene'] = {'scene':'multimodifiers', 'time':now};
    ...
    applyRemoteState = (state) => {
        ...
        if(state[remoteId+'scene']){
            if(state[remoteId+'scene'].scene !== 'multimodifiers' && this.playerId!=='p1'){
    ...
    update(delta) {
        ...
        this.dragons.forEach((dragon)=>{
            ...
            if(dragon.pos.x<60&&dragon.pos.y>1030) this.dragonsOnButton+=1;
        })
